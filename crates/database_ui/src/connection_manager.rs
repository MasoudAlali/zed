...
impl ConnectionManager {
    ...
    pub fn update_connection(
        ...
        let old_name = self.connections[index].config.name.clone();
        let old_id = self.connections[index].config.id.clone();
        unregister_connection(&old_name);
        close_tunnel(&old_id);
        ...
        self.connections[index] = ConnectionEntry {
            config: config.clone(),
            connection: None,
            schema: None,
            error: None,
        ...
        self.connect_task = cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                match result {
                    ...
                    Err(error) => {
                        let error_msg = format!("{:#}", error);
                        log::error!("database_viewer: connection update failed: {}", error_msg);
                        if let Some(entry) = this.connections.get_mut(index) {
                            entry.error = Some(error_msg.clone());
                        }
                        cx.emit(ConnectionManagerEvent::ConnectionFailed {
                    ...
                    if !alive {
                        ...
                        let reconnect_info = this
                            .update(cx, |this, cx| {
                                if let Some(entry) = this.connections.get_mut(index) {
                                    ...
                                    entry.error = Some("Connection lost".to_string());
                                }
                        ...
                        loop {
                            if retries >= MAX_RETRIES {
                                log::error!(
                                    "database_viewer: giving up reconnect for connection {} after {} attempts",
                                    index, MAX_RETRIES
                                );
                                this.update(cx, |this, cx| {
                                    if let Some(entry) = this.connections.get_mut(index) {
                                        entry.error = Some("Reconnect failed after multiple attempts".to_string());
                                    }
                                    cx.notify();
                                }).log_err();
                                break;
