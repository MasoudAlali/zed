...
use fs::Fs;
use gpui::{
    actions, anchored, deferred, div, prelude::*, px, Anchor, App, AsyncWindowContext,
    ClipboardItem, Context, DismissEvent, Entity, EventEmitter, ExternalPaths, FocusHandle,
    Focusable,
...
use ui::{
    prelude::*, Banner, Button, ButtonStyle, ContextMenu, Icon, IconButton, IconName, IconSize,
    Label, PopoverMenu, Severity, Tooltip,
};
...
use crate::connection_dialog::{ConnectionDialog, ConnectionDialogEvent, ShowConnectionDialog, CONNECTION_COLORS};
...
pub fn set_editor_sql_language(
    editor: &Entity<Editor>,
    workspace: &Workspace,
    cx: &mut App,
) {
    let languages = workspace.project().read(cx).languages().clone();
    let sql_path = std::path::Path::new("query.sql");

    let language_task = languages.load_language_for_file_path(sql_path);
    ...
    if let Some(buffer) = buffer {
        buffer.update(cx, |_buffer, cx| {
            cx.spawn(async move |this, cx| {
                match language_task.await {
                    Ok(language) => {
                    ...
                    Err(error) => {
                        log::warn!("database_viewer: failed to load SQL language (install the SQL extension for syntax highlighting): {:#}", error);
                    }
                }
            }).detach();
        });
...
struct SerializedDatabasePanel {
    ...
    #[serde(default = "default_true")]
    schema_section_expanded: bool,
    #[serde(default)]
    query_section_collapsed: bool,
}
...
impl DatabasePanel {
    fn new(
        ...
        let panel = cx.new(|cx| {
            ...
            let query_editor = cx.new(|cx| {
                let mut editor = Editor::auto_height(8, 20, window, cx);
                editor.set_placeholder_text("Enter SQL query...", window, cx);
                editor
            });

            set_editor_sql_language(&query_editor, workspace, cx);
        ...
        workspace.update_in(&mut cx, |workspace, window, cx| {
            ...
            if let Some(serialized) = serialized_panel {
                panel.update(cx, |panel, cx| {
                    ...
                    panel.schema_section_expanded = serialized.schema_section_expanded;
                    panel.query_section_collapsed = serialized.query_section_collapsed;
    ...
    fn serialize(&self, cx: &mut Context<Self>) {
        ...
        let serialized_panel = SerializedDatabasePanel {
            ...
            schema_section_expanded: self.schema_section_expanded,
            query_section_collapsed: self.query_section_collapsed,
        };
    ...
    fn show_postgres_connection_form(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        self.open_connection_dialog("postgresql", window, cx);
    }

    fn show_mysql_connection_form(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        self.open_connection_dialog("mysql", window, cx);
    }
    ...
    fn open_connection_dialog(
        ...
        workspace.update(cx, |workspace, cx| {
            workspace.toggle_modal(window, cx, |window, cx| {
                ConnectionDialog::new_with_tab(&tab, window, cx)
            });

            if let Some(modal) = workspace.active_modal::<ConnectionDialog>(cx) {
                let subscription = cx.subscribe(&modal, move |_workspace, _dialog, event, cx| {
                    match event {
                        ConnectionDialogEvent::ConnectionCreated(config) => {
                            if let Some(panel) = panel_handle.upgrade() {
                                let config = config.clone();
                                panel.update(cx, |this, cx| {
                                    let index = this.add_connection(config, cx);
                                    this.connection_manager.update(cx, |manager, cx| {
                                        manager.set_active_connection(Some(index), cx);
                                    });
                                });
                        ...
                        ConnectionDialogEvent::ConnectionUpdated { index, config } => {
                            if let Some(panel) = panel_handle.upgrade() {
                                let config = config.clone();
                                let index = *index;
                                panel.update(cx, |this, cx| {
                                    ...
                                    this.connection_manager.update(cx, |manager, cx| {
                                        manager.update_connection(index, config, cx);
                                    });
                                });
                            }
                        }
    ...
    fn show_connection_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, move |menu, _window, _cx| {
            ...
            menu = menu.entry("Edit Connection", None, {
                let handle = handle.clone();
                move |window, cx| {
                    if let Some(panel) = handle.upgrade() {
                        panel.update(cx, |this, cx| {
                            this.edit_connection(index, window, cx);
                        });
                    }
                }
            });
    ...
    fn edit_connection(
        ...
        workspace.update(cx, |workspace, cx| {
            workspace.toggle_modal(window, cx, |window, cx| {
                let mut dialog = ConnectionDialog::new(window, cx);
                dialog.load_config(&config, index, window, cx);
                dialog
            });
            ...
            if let Some(modal) = workspace.active_modal::<ConnectionDialog>(cx) {
                let subscription = cx.subscribe(&modal, move |_workspace, _dialog, event, cx| {
                    match event {
                        ...
                        ConnectionDialogEvent::ConnectionCreated(_) => {}
                    }
                });
                subscription.detach();
            }
    ...
    fn execute_query(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        let manager = self.connection_manager.read(cx);
        ...
        if let Some(workspace) = self._workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
                let tab = cx.new(|cx| {
                    crate::query_editor::QueryEditorTab::new(
                        connection_manager,
                        Some(sql),
                        None,
                        window,
                ...
                tab.update(cx, |tab, cx| {
                    tab.set_sql_language(workspace, cx);
                    tab.run_query(window, cx);
                });
    ...
    fn explain_query(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        ...
        self.query_history.push(&explain_sql);
        ...
        h_flex()
            ...
            .child(
                h_flex()
                    ...
                    .items_center()
                    .child(
                        IconButton::new("page-first", IconName::ArrowLeft)
                    ...
                    .child(
                        IconButton::new("page-last", IconName::ArrowRight)
                            .icon_size(IconSize::XSmall)
                            .style(ButtonStyle::Subtle)
                            .disabled(is_last_page)
    ...
    fn render_saved_queries_panel(
        ...
        panel = panel.child(
            h_flex()
                ...
                .child(
                    IconButton::new("close-saved-queries", IconName::Close)
                        .icon_size(IconSize::XSmall)
                        .style(ButtonStyle::Subtle)
                        .on_click(cx.listener(|this, _, _window, cx| {
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                IconButton::new("refresh-schema", IconName::ArrowCircle)
                    ...
                    let menu = ContextMenu::build(window, cx, move |menu, _window, _cx| {
                        ...
                        .entry("Connect to PostgreSQL...", None, {
                            ...
                            move |window, cx| {
                                if let Some(panel) = handle.upgrade() {
                                    panel.update(cx, |this, cx| {
                                        this.show_postgres_connection_form(window, cx);
                                    });
                                }
                            }
                        })
                        .entry("Connect to MySQL...", None, {
                            ...
                            move |window, cx| {
                                if let Some(panel) = handle.upgrade() {
                                    panel.update(cx, |this, cx| {
                                        this.show_mysql_connection_form(window, cx);
                                    });
                                }
                            }
                        })
                        .entry("Connect to Redis...", None, {
                            move |window, cx| {
                                if let Some(panel) = handle.upgrade() {
                                    panel.update(cx, |this, cx| {
                                        this.open_connection_dialog("redis", window, cx);
                                    });
                                }
                            }
                        })
    ...
    fn render_schema_section(
        ...
        let is_expanded = self.schema_section_expanded;
        ...
        let mut section = v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .hover(|style| style.bg(cx.theme().colors().ghost_element_hover))
                    .on_click(cx.listener(|this, _, _window, cx| {
                        this.schema_section_expanded = !this.schema_section_expanded;
                        this.serialize(cx);
                        cx.notify();
                    }))
                    .child(
                        h_flex()
                            ...
                            .child(
                                Label::new(format!("Schema ({})", table_count))
                                    .size(LabelSize::Small)
                                    .weight(gpui::FontWeight::BOLD),
                            ),
                    ),
                                ...
                                move |index, position, window, cx| {
                                    entity_for_secondary
                                        .update(cx, |panel, cx| {
                                            ...
                                            panel.show_schema_context_menu(
    ...
    fn render_query_section(
        ...
        let is_collapsed = self.query_section_collapsed;
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .hover(|style| style.bg(cx.theme().colors().ghost_element_hover))
                    .on_click(cx.listener(|this, _, _window, cx| {
                        this.query_section_collapsed = !this.query_section_collapsed;
                        this.serialize(cx);
                        cx.notify();
                    }))
                    .child(
                    ...
                    .when(!is_collapsed, |this| {
                        this.child(
                            h_flex()
                                .gap_1()
                                .child(
                                    IconButton::new("history-prev", IconName::ArrowUp)
                                        .icon_size(IconSize::XSmall)
                                        .style(ButtonStyle::Subtle)
                                ...
                                .child(
                                    IconButton::new("history-next", IconName::ArrowDown)
                                        .icon_size(IconSize::XSmall)
                                        .style(ButtonStyle::Subtle)
                                ...
                                .child(
                                    IconButton::new("save-query-btn", IconName::Star)
                                        .icon_size(IconSize::XSmall)
                                        .style(ButtonStyle::Subtle)
                                ...
                                .child(
                                    IconButton::new("show-saved-btn", IconName::StarFilled)
                                        .icon_size(IconSize::XSmall)
                                ...
                                .child(
                                    Button::new("run-query", "Run")
                                        ...
                                        .when(
                                            ...
                                            |this: Button| this.disabled(true),
                                        )
                                        .on_click(cx.listener(|this, _, window, cx| {
                                            this.execute_query(window, cx);
                                        })),
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            .gap_1()
                            .child(
                                IconButton::new("copy-csv", IconName::Copy)
                                    .icon_size(IconSize::XSmall)
                                    .style(ButtonStyle::Subtle)
                            ...
                            .child(
                                IconButton::new("copy-json", IconName::FileCode)
                                    .icon_size(IconSize::XSmall)
                                    .style(ButtonStyle::Subtle)
                                    .disabled(!has_results)
                            ...
                            .child(
                                IconButton::new("export-dialog", IconName::Download)
                                    .icon_size(IconSize::XSmall)
                                    .style(ButtonStyle::Subtle)
                                    .disabled(!has_results)
                            ...
                            .child(
                                IconButton::new("pin-tab", IconName::Pin)
                                    .icon_size(IconSize::XSmall)
                                    .style(ButtonStyle::Subtle)
                                    .disabled(!has_results)
...
impl Render for DatabasePanel {
    fn render(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_action(cx.listener(|this, _: &ConnectPostgresql, window, cx| {
                this.show_postgres_connection_form(window, cx);
            }))
            .on_action(cx.listener(|this, _: &ConnectMysql, window, cx| {
                this.show_mysql_connection_form(window, cx);
            }))
...
impl Panel for DatabasePanel {
    ...
    fn default_size(&self, _window: &gpui::Window, cx: &App) -> Pixels {
