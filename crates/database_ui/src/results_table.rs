...
use gpui::{div, prelude::*, px, App, CursorStyle, Entity, IntoElement, MouseButton, MouseDownEvent, Pixels, Point, SharedString, UniformListScrollHandle};
...
pub fn render_results_table(
    ...
    editing_cell: Option<(usize, usize, Entity<editor::Editor>)>,
    ...
    let total_width = config.total_width(column_count) + px(ROW_NUMBER_WIDTH);
                ...
                move |range, _window, cx| {
                    range
                        .map(|index| {
                            ...
                            let cell_editor = editing_cell.as_ref().and_then(|(r, c, editor)| {
                                if *r == index {
                                    Some((*c, editor.clone()))
                                } else {
                                    None
...
fn render_data_row(
    ...
    col_widths: &[f32],
    selected_cell: Option<(usize, usize)>,
    cell_editor: Option<(usize, Entity<editor::Editor>)>,
    ...
    let mut row_element = div()
        ...
        .child(
            div()
                ...
                .child(
                    Label::new(SharedString::from(format!("{}", absolute_row + 1)))
                        .size(LabelSize::Small)
                        .color(ui::Color::Muted),
                ),
        );
    ...
    for (col_index, cell) in row.iter().enumerate() {
        ...
        let is_editing = cell_editor.as_ref().map(|(c, _)| *c == col_index).unwrap_or(false);
        ...
        let cell_content: gpui::AnyElement = if is_editing {
            if let Some((_, editor)) = &cell_editor {
                div()
                    .w_full()
                    .child(editor.clone())
                    .into_any_element()
        ...
        row_element = row_element.child(
            div()
                ...
                .when(is_selected || is_editing, |this| {
                    this.border_1()
