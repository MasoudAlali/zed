...
use ui::{prelude::*, Button, ButtonStyle, Icon, IconButton, IconName, IconSize, Label, Tooltip};
...
impl ResultGrid {
    ...
    fn delete_selected_rows(&mut self, cx: &mut Context<Self>) {
        ...
        if marked_any {
            self.show_pending_details = true;
        }
        cx.notify();
    }
    ...
    fn revert_pending_edits(&mut self, cx: &mut Context<Self>) {
        ...
        self.pending_row_states.clear();
        self.edit_history.clear();
        self.show_pending_details = false;
    ...
    fn render_grid_view(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let editing_cell_for_table = self.editing_cell.and_then(|(row, col)| {
            self.cell_editor.as_ref().map(|editor| (row, col, editor.clone()))
        });
    ...
    fn render_record_view(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let navigation_bar = h_flex()
            ...
            .items_center()
            .child(
                IconButton::new("record-prev", IconName::ChevronLeft)
            ...
            .child(
                IconButton::new("record-next", IconName::ChevronRight)
                    .icon_size(IconSize::Small)
                    .style(ButtonStyle::Subtle)
    ...
    fn render_pending_edits_bar(&self, cx: &mut Context<Self>) -> Option<gpui::AnyElement> {
        ...
        let chevron_icon = if self.show_pending_details {
        ...
        let mut container = v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        h_flex()
                            .id("pending-edits-toggle")
                            .gap_1()
                            .items_center()
                            .cursor_pointer()
                            .on_click(cx.listener(|this, _, _window, cx| {
                                this.show_pending_details = !this.show_pending_details;
                                cx.notify();
                            }))
                            .child(
        ...
        if self.show_pending_details {
            container = container.child(self.render_pending_details(cx));
        }

        Some(container.into_any_element())
    }

    fn render_pending_details(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        let result = self.result.as_ref();

        let mut details = v_flex()
            .id("pending-details-scroll")
            .w_full()
            .max_h(px(200.0))
            .overflow_y_scroll()
            .px_2()
        ...
        for edit in &self.pending_edits {
            let column_name = result
                .and_then(|r| r.columns.get(edit.col))
                .cloned()
                .unwrap_or_else(|| format!("col{}", edit.col));
        ...
        for (row_index, state) in &self.pending_row_states {
            let (icon, description) = match state {
                PendingRowState::Insert => (
                    IconName::Plus,
                    format!("Row {} \u{00B7} New row", row_index + 1),
                ),
                PendingRowState::Delete => (
                    IconName::Trash,
                    format!("Row {} \u{00B7} Delete", row_index + 1),
                ),
            };

            let icon_color = match state {
                PendingRowState::Insert => ui::Color::Success,
                PendingRowState::Delete => ui::Color::Error,
            };
    ...
    fn render_where_filter_bar(&self, cx: &mut Context<Self>) -> Option<gpui::AnyElement> {
        ...
        Some(
            h_flex()
                ...
                .child(
                    IconButton::new("clear-where", IconName::Close)
                        .icon_size(IconSize::XSmall)
                        .style(ButtonStyle::Subtle)
                        .on_click(cx.listener(|this, _, _window, cx| {
    ...
    fn render_view_mode_toolbar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        h_flex()
            ...
            .border_color(cx.theme().colors().border)
            .child(
                IconButton::new("grid-view", IconName::ListTree)
                    .icon_size(IconSize::Small)
            ...
            .child(
                IconButton::new("record-view", IconName::Notepad)
                    .icon_size(IconSize::Small)
            ...
            .child(
                h_flex()
                    .gap_1()
                    .child(
                        IconButton::new("add-row-btn", IconName::Plus)
                            .icon_size(IconSize::Small)
                            .style(ButtonStyle::Subtle)
                            .on_click(cx.listener(|this, _, _window, cx| {
                                this.add_row(cx);
                    ...
                    .child(
                        IconButton::new("delete-rows-btn", IconName::Trash)
                            .icon_size(IconSize::Small)
                            .style(ButtonStyle::Subtle)
                    ...
                    .child(
                        IconButton::new("clone-row-btn", IconName::Copy)
                            .icon_size(IconSize::Small)
                            .style(ButtonStyle::Subtle)
                    ...
                    .when(self.edit_history.can_undo(), |el| {
                        el.child(
                            IconButton::new("undo-btn", IconName::Undo)
                                .icon_size(IconSize::Small)
                                .style(ButtonStyle::Subtle)
                                .on_click(cx.listener(|this, _, _window, cx| {
                                    this.undo_edit(cx);
                    ...
                    .when(self.edit_history.can_redo(), |el| {
                        el.child(
                            IconButton::new("redo-btn", IconName::Return)
                                .icon_size(IconSize::Small)
                                .style(ButtonStyle::Subtle)
                                .on_click(cx.listener(|this, _, _window, cx| {
                                    this.redo_edit(cx);
