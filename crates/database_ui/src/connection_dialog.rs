...
use ui::{prelude::*, Banner, Button, ButtonStyle, Icon, IconButton, IconName, IconSize, Label, Severity, Tooltip};
...
impl DriverTab {
    fn label(&self) -> &'static str {
        match self {
            ...
            DriverTab::Redis => "Redis",
        }
    }

    fn all() -> &'static [DriverTab] {
        &[DriverTab::Sqlite, DriverTab::PostgreSql, DriverTab::MySql, DriverTab::Redis]
    }
}
...
impl ConnectionDialog {
    ...
    pub fn load_config(
        ...
        self.active_tab = match config.database_type {
            database_core::DatabaseType::Sqlite => DriverTab::Sqlite,
            database_core::DatabaseType::PostgreSql => DriverTab::PostgreSql,
            database_core::DatabaseType::MySql => DriverTab::MySql,
            database_core::DatabaseType::Redis => DriverTab::Redis,
        };

        self.name_field.update(cx, |editor, cx| {
            editor.set_text(config.name.clone(), window, cx);
        });

        if let Some(path) = &config.path {
            self.path_field.update(cx, |editor, cx| {
                editor.set_text(path.to_string_lossy().to_string(), window, cx);
            });
        }
        if let Some(host) = &config.host {
            self.host_field.update(cx, |editor, cx| {
                editor.set_text(host.clone(), window, cx);
            });
        }
        ...
        if let Some(database) = &config.database {
            self.database_field.update(cx, |editor, cx| {
                editor.set_text(database.clone(), window, cx);
            });
        }
        if let Some(user) = &config.user {
            self.user_field.update(cx, |editor, cx| {
                editor.set_text(user.clone(), window, cx);
            });
        }
        if let Some(password) = &config.password {
            self.password_field.update(cx, |editor, cx| {
                editor.set_text(password.clone(), window, cx);
            });
        }

        self.ssl_mode = config.ssl_mode;
        self.selected_color = config.color_index;
        self.read_only = config.read_only;
        self.introspection_level = config.introspection_level;

        if let Some(ssl_config) = &config.ssl_config {
            if let Some(path) = &ssl_config.ca_cert_path {
                self.ssl_ca_cert_field.update(cx, |editor, cx| {
                    editor.set_text(path.to_string_lossy().to_string(), window, cx);
                });
            }
            if let Some(path) = &ssl_config.client_cert_path {
                self.ssl_client_cert_field.update(cx, |editor, cx| {
                    editor.set_text(path.to_string_lossy().to_string(), window, cx);
                });
            }
            if let Some(path) = &ssl_config.client_key_path {
                self.ssl_client_key_field.update(cx, |editor, cx| {
                    editor.set_text(path.to_string_lossy().to_string(), window, cx);
                });
            }
        }
        ...
        if let Some(ssh) = &config.ssh_tunnel {
            ...
            self.ssh_host_field.update(cx, |editor, cx| {
                editor.set_text(ssh.host.clone(), window, cx);
            });
            self.ssh_port_field.update(cx, |editor, cx| {
                editor.set_text(ssh.port.to_string(), window, cx);
            });
            self.ssh_user_field.update(cx, |editor, cx| {
                editor.set_text(ssh.username.clone(), window, cx);
            });
            match &ssh.auth_method {
                ...
                SshAuthMethod::PrivateKey { key_path, passphrase } => {
                    ...
                    self.ssh_key_path_field.update(cx, |editor, cx| {
                        editor.set_text(key_path.to_string_lossy().to_string(), window, cx);
                    });
                    if let Some(passphrase) = passphrase {
                        self.ssh_passphrase_field.update(cx, |editor, cx| {
                            editor.set_text(passphrase.clone(), window, cx);
                        });
                    }
                }
                SshAuthMethod::Agent => {
                    self.ssh_auth_method = SshAuthSelection::Agent;
                }
            }
        }
    ...
    pub fn new_with_tab(
        ...
        let mut dialog = Self::new(window, cx);
        let driver_tab = match tab {
            "sqlite" => DriverTab::Sqlite,
            "mysql" => DriverTab::MySql,
            "redis" => DriverTab::Redis,
            _ => DriverTab::PostgreSql,
        };
        dialog.set_tab(driver_tab, window, cx);
        dialog
    }
    ...
    fn build_config(&self, cx: &App) -> Result<ConnectionConfig, String> {
        ...
        let mut config = match self.active_tab {
            ...
            DriverTab::Redis => {
                ...
                let user_text = self.user_field.read(cx).text(cx).trim().to_string();
                let user = if user_text.is_empty() { None } else { Some(user_text) };
                let password_text = self.password_field.read(cx).text(cx);
                let password = if password_text.is_empty() { None } else { Some(password_text) };
                let name = if name.is_empty() {
                    format!("Redis @ {}:{}", host, port)
                } else {
                    name
                };
                ConnectionConfig::redis(name, host, port, user, password)
            }
        };
    ...
    fn save_connection(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(index) = self.editing_index {
            cx.emit(ConnectionDialogEvent::ConnectionUpdated { index, config });
        } else {
...
impl Render for ConnectionDialog {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let is_network_driver = matches!(self.active_tab, DriverTab::PostgreSql | DriverTab::MySql | DriverTab::Redis);
        ...
        let mut content = v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        IconButton::new("close-dialog", ui::IconName::Close)
        ...
        if is_network_driver {
            let is_redis = matches!(self.active_tab, DriverTab::Redis);
            content = content
                .child(
                    h_flex()
                        .gap_2()
            ...
            content = content
                .child(Self::render_field(
                    if is_redis { "User (optional)" } else { "User" },
                    &self.user_field,
                    cx,
                ))
                .child(Self::render_field(
                    if is_redis { "Password (optional)" } else { "Password" },
                    &self.password_field,
                    cx,
                ));
            if !is_redis {
