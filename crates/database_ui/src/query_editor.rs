...
impl QueryEditorTab {
    ...
    pub fn set_sql_language(&self, workspace: &Workspace, cx: &mut App) {
        crate::database_panel::set_editor_sql_language(&self.editor, workspace, cx);
    }

    pub fn run_query(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
    ...
    fn explain_query(&mut self, window: &mut gpui::Window, cx: &mut Context<Self>) {
        ...
        let explain_sql = match db_type {
            ...
            DatabaseType::Redis => {
                self.query_error = Some("EXPLAIN is not supported for Redis".to_string());
                cx.notify();
                return;
            }
        };
    ...
    fn save_history(&self, cx: &App) {
        ...
        if let Ok(json) = serde_json::to_string(&entries) {
            ...
            db::write_and_log(cx, move || async move { kvp.write_kvp(key, json).await });
        }
    }
    ...
    fn load_history(&mut self, cx: &App) {
        ...
        let key = format!("database_query_history-db_history_{}", connection_name);
        if let Ok(Some(json)) = KeyValueStore::global(cx).read_kvp(&key) {
        ...
        h_flex()
            ...
            .child(
                Button::new("format", "Format")
                    .style(ButtonStyle::Subtle)
                    .start_icon(Icon::new(IconName::TextSnippet).size(IconSize::Small))
                    .on_click(cx.listener(|this, _, window, cx| {
            ...
            .child(
                IconButton::new("pin-tab", if self.is_pinned { IconName::Unpin } else { IconName::Pin })
            ...
            .child(
                IconButton::new("execute-to-file", IconName::Download)
                    .icon_size(IconSize::Small)
                    .style(ButtonStyle::Subtle)
                    .disabled(is_executing)
                    .on_click(cx.listener(|this, _, _window, cx| {
