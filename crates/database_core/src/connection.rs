...
pub fn quote_identifier(name: &str, database_type: &DatabaseType) -> String {
    match database_type {
        ...
        DatabaseType::Redis => name.to_string(),
    }
}
...
impl ConnectionConfig {
    ...
    pub fn from_redis_url(connection_string: &str) -> Result<Self, String> {
        let trimmed = connection_string.trim();
        if trimmed.is_empty() {
            return Err("Connection string is required.".to_string());
        }

        let normalized = if !trimmed.starts_with("redis://") {
            format!("redis://{}", trimmed)
        } else {
            trimmed.to_string()
        };

        let for_parsing = normalized.replacen("redis://", "http://", 1);
        let parsed = url::Url::parse(&for_parsing)
            .map_err(|_| "Invalid Redis connection string".to_string())?;
        ...
        let user = if parsed.username().is_empty() {
            ...
            Some(percent_encoding::percent_decode_str(parsed.username())
                .decode_utf8_lossy()
                .to_string())
        };
        ...
        let name = format!("Redis @ {}:{}", host, port);

        Ok(Self::redis(name, host, port, user, password))
    }
...
pub struct RedisConnection {
    client: std::sync::Mutex<redis::Client>,
...
impl RedisConnection {
    pub fn connect(config: &ConnectionConfig) -> Result<Self> {
        ...
        let connection_url = if let Some(password) = &config.password {
    ...
    fn get_connection(&self) -> Result<redis::Connection> {
        let client = self.client.lock().map_err(|e| anyhow::anyhow!("Lock error: {}", e))?;
        client
            .get_connection()
            .map_err(|e| anyhow::anyhow!("Redis connection error: {}", e))
    }

    fn scan_keys(&self, pattern: &str, count: usize) -> Result<Vec<String>> {
        ...
        let keys: Vec<String> = iter.take(count).collect();
        Ok(keys)
    }

    fn get_key_type(&self, key: &str) -> Result<String> {
        let mut conn = self.get_connection()?;
        let key_type: String = redis::cmd("TYPE")
            .arg(key)
            .query(&mut conn)
            .map_err(|e| anyhow::anyhow!("Redis TYPE error: {}", e))?;
        Ok(key_type)
    }

    fn get_key_value(&self, key: &str, key_type: &str) -> Result<Vec<CellValue>> {
        let mut conn = self.get_connection()?;
        match key_type {
            "string" => {
            ...
            "list" => {
                let values: Vec<String> = redis::cmd("LRANGE")
                    ...
                    .query(&mut conn)
                    .unwrap_or_default();
                Ok(vec![CellValue::Text(format!("[{}]", values.join(", ")))])
            }
            "set" => {
            ...
            "zset" => {
                ...
                let formatted: Vec<String> = values
                    ...
                    .collect();
                Ok(vec![CellValue::Text(format!("{{{}}}", formatted.join(", ")))])
            }
            "hash" => {
            ...
            "stream" => {
                Ok(vec![CellValue::Text("(stream)".to_string())])
            }
            _ => {
                Ok(vec![CellValue::Text(format!("({})", key_type))])
            }
        }
    }

    fn get_key_ttl(&self, key: &str) -> Result<i64> {
        let mut conn = self.get_connection()?;
    ...
    fn execute_redis_command(&self, command_str: &str) -> Result<QueryResult> {
        ...
        let result: redis::RedisResult<redis::Value> = cmd.query(&mut conn);

        match result {
            Ok(value) => {
                let execution_time = start.elapsed();
                Ok(redis_value_to_query_result(value, execution_time))
            }
...
fn redis_value_to_query_result(value: redis::Value, execution_time: std::time::Duration) -> QueryResult {
    match value {
        ...
        redis::Value::Int(i) => QueryResult {
            columns: vec!["result".to_string()],
            rows: vec![vec![CellValue::Integer(i)]],
            total_row_count: Some(1),
            affected_rows: None,
            execution_time,
        },
        redis::Value::BulkString(bytes) => {
            ...
            QueryResult {
                columns: vec!["result".to_string()],
                rows: vec![vec![CellValue::Text(text)]],
                total_row_count: Some(1),
                affected_rows: None,
                execution_time,
            }
            ...
            let rows: Vec<Vec<CellValue>> = items
                ...
                .map(|(index, item)| {
                    let value = match item {
                        ...
                        redis::Value::Int(i) => CellValue::Integer(i),
                        redis::Value::BulkString(bytes) => {
                            CellValue::Text(String::from_utf8_lossy(&bytes).to_string())
                        }
                        redis::Value::SimpleString(s) => CellValue::Text(s),
                        redis::Value::Array(nested) => {
                            let nested_strs: Vec<String> = nested
                                .into_iter()
                                .map(|v| redis_value_to_string(v))
                                .collect();
                            CellValue::Text(format!("[{}]", nested_strs.join(", ")))
                        }
                        other => CellValue::Text(redis_value_to_string(other)),
                    };
                    vec![CellValue::Integer(index as i64), value]
                })
                .collect();
        ...
        other => QueryResult {
            columns: vec!["result".to_string()],
            rows: vec![vec![CellValue::Text(redis_value_to_string(other))]],
            total_row_count: Some(1),
            affected_rows: None,
            execution_time,
        },
...
fn redis_value_to_string(value: redis::Value) -> String {
    match value {
        ...
        redis::Value::Array(items) => {
            let strs: Vec<String> = items.into_iter().map(redis_value_to_string).collect();
            format!("[{}]", strs.join(", "))
        }
...
impl DatabaseConnection for RedisConnection {
    fn fetch_schema_with_level(&self, _level: IntrospectionLevel) -> Result<DatabaseSchema> {
        ...
        for key in &keys {
            if let Ok(key_type) = self.get_key_type(key) {
        ...
        // Create a virtual "keys" table for browsing all keys
        tables.push(TableInfo {
            name: "keys".to_string(),
            columns: vec![
                ColumnInfo {
        ...
        // Create a virtual table per key type
        for (key_type, count) in &type_counts {
            let type_columns = match key_type.as_str() {
                "hash" => vec![
                    ColumnInfo { name: "key".to_string(), data_type: "string".to_string(), nullable: false, primary_key: true, default_value: None },
                    ColumnInfo { name: "field".to_string(), data_type: "string".to_string(), nullable: false, primary_key: false, default_value: None },
                    ColumnInfo { name: "value".to_string(), data_type: "string".to_string(), nullable: true, primary_key: false, default_value: None },
                ],
                _ => vec![
                    ColumnInfo { name: "key".to_string(), data_type: "string".to_string(), nullable: false, primary_key: true, default_value: None },
                    ColumnInfo { name: "value".to_string(), data_type: "string".to_string(), nullable: true, primary_key: false, default_value: None },
            ...
            tables.push(TableInfo {
                name: format!("{}s", key_type),
                columns: type_columns,
                indexes: Vec::new(),
                foreign_keys: Vec::new(),
                row_count: Some(*count as u64),
                table_kind: TableKind::VirtualTable,
                ddl: None,
            });
        }

        // Add server info as a table
        ...
        if !server_info.is_empty() {
            tables.push(TableInfo {
                ...
                columns: vec![
                    ColumnInfo { name: "property".to_string(), data_type: "string".to_string(), nullable: false, primary_key: true, default_value: None },
                    ColumnInfo { name: "value".to_string(), data_type: "string".to_string(), nullable: true, primary_key: false, default_value: None },
                ],
                indexes: Vec::new(),
                foreign_keys: Vec::new(),
                row_count: None,
                table_kind: TableKind::View,
                ddl: Some(info),
            });
        }
    ...
    fn execute_query(&self, command: &str) -> Result<QueryResult> {
        let trimmed = command.trim();

        // Handle virtual table queries: SELECT * FROM keys / SELECT * FROM server_info
        if let Some(table_query) = parse_redis_virtual_query(trimmed) {
            return self.execute_virtual_query(&table_query);
        }

        self.execute_redis_command(trimmed)
    }

    fn execute_query_paged(&self, command: &str, limit: usize, offset: usize) -> Result<QueryResult> {
        let trimmed = command.trim();

        if let Some(table_query) = parse_redis_virtual_query(trimmed) {
            return self.execute_virtual_query_paged(&table_query, limit, offset);
        }

        self.execute_redis_command(trimmed)
    }

    fn execute_statement(&self, command: &str) -> Result<u64> {
        let result = self.execute_redis_command(command)?;
        Ok(result.affected_rows.unwrap_or(0))
    }

    fn interrupt(&self) {
        // Redis operations are typically fast and don't need interruption
    }
...
struct RedisVirtualQuery {
    table: String,
    pattern: Option<String>,
}

fn parse_redis_virtual_query(query: &str) -> Option<RedisVirtualQuery> {
    let upper = query.trim().to_uppercase();
    if !upper.starts_with("SELECT") {
        return None;
    }

    // Simple parsing: SELECT * FROM <table> [WHERE key LIKE '<pattern>']
    let lower = query.trim().to_lowercase();
    let from_pos = lower.find(" from ")?;
    let rest = &lower[from_pos + 6..].trim();

    let table = rest.split_whitespace().next()?.to_string();

    let pattern = if let Some(where_pos) = lower.find(" where ") {
        let where_clause = &query.trim()[where_pos + 7..].trim();
        if let Some(like_pos) = where_clause.to_lowercase().find("like") {
            let pattern_str = where_clause[like_pos + 4..].trim().trim_matches('\'').trim_matches('"');
            // Convert SQL LIKE to Redis glob: % → *, _ → ?
            let redis_pattern = pattern_str.replace('%', "*").replace('_', "?");
            Some(redis_pattern)
        } else {
            None
        }
    ...
    Some(RedisVirtualQuery { table, pattern })
}

impl RedisConnection {
    fn execute_virtual_query(&self, query: &RedisVirtualQuery) -> Result<QueryResult> {
        self.execute_virtual_query_paged(query, 1000, 0)
    }

    fn execute_virtual_query_paged(
        &self,
        query: &RedisVirtualQuery,
        limit: usize,
        offset: usize,
    ) -> Result<QueryResult> {
        let start = std::time::Instant::now();
        ...
        match query.table.as_str() {
            "keys" => {
                ...
                let keys = self.scan_keys(pattern, limit + offset + 100)?;

                let mut rows = Vec::new();
                let total = keys.len();

                for key in keys.into_iter().skip(offset).take(limit) {
                    let key_type = self.get_key_type(&key).unwrap_or_else(|_| "unknown".to_string());
                    let value = self.get_key_value(&key, &key_type)
                        .map(|v| v.into_iter().next().unwrap_or(CellValue::Null))
                        .unwrap_or(CellValue::Null);
                    let ttl = self.get_key_ttl(&key).unwrap_or(-1);
                    let ttl_cell = if ttl < 0 {
                        CellValue::Null
                    } else {
                        CellValue::Integer(ttl)
                    };

                    rows.push(vec![
                        CellValue::Text(key),
                        CellValue::Text(key_type),
                        value,
                        ttl_cell,
                    ]);
                }
                ...
                Ok(QueryResult {
                    ...
                    rows,
                    total_row_count: Some(total as u64),
                    affected_rows: None,
            ...
            "server_info" => {
                ...
                let rows: Vec<Vec<CellValue>> = info
                    .lines()
                    ...
                    .filter_map(|line| {
                        ...
                        Some(vec![CellValue::Text(key), CellValue::Text(value)])
                    })
                    .skip(offset)
                    .take(limit)
                    .collect();
            ...
            _ => {
                // For type-specific virtual tables (strings, hashs, etc.)
                let type_name = query.table.trim_end_matches('s');
                let pattern = query.pattern.as_deref().unwrap_or("*");
                let all_keys = self.scan_keys(pattern, 10000)?;

                let mut rows = Vec::new();
                let mut count = 0;
                for key in all_keys {
                    let key_type = self.get_key_type(&key).unwrap_or_default();
                ...
                Ok(QueryResult {
                    ...
                    rows,
                    total_row_count: Some(count as u64),
                    affected_rows: None,
                    execution_time: start.elapsed(),
                })
            }
...
impl DatabaseDriver for RedisDriver {
    ...
    fn validate_config(&self, config: &ConnectionConfig) -> Result<(), String> {
        if config.host.is_none() || config.host.as_deref() == Some("") {
            return Err("Redis connection requires a host".to_string());
        }
        Ok(())
    }

    fn connect(&self, config: &ConnectionConfig) -> Result<Arc<dyn DatabaseConnection>> {
        let connection = RedisConnection::connect(config)?;
        Ok(Arc::new(connection))
    }
}
...
impl DriverRegistry {
    pub fn new() -> Self {
        ...
        registry.register(Arc::new(RedisDriver));
        registry
    }
...
impl Drop for SshTunnel {
    fn drop(&mut self) {
        if let Err(error) = self.process.kill() {
            log::warn!("Failed to kill SSH tunnel process: {error}");
        }
    }
}
...
fn make_placeholder(db_type: &DatabaseType, index: usize) -> String {
    match db_type {
        ...
        DatabaseType::Sqlite | DatabaseType::MySql | DatabaseType::Redis => "?".to_string(),
    }
}
