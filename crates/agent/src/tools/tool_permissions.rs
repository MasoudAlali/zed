...
pub async fn resolve_global_skill_path(path: &Path, fs: &dyn Fs) -> Option<PathBuf> {
    ...
    // the symlink target. A linked immediate skill directory is allowed
    // separately, but only for paths that stay under that skill target.
    let canonical_path = fs.canonicalize(&normalized_path).await.ok()?;

    // Check every global skills root (`~/.agents/skills` and
    // `~/.claude/skills`). For each, the path is allowed if it canonicalizes
    // under that root, or if it lives under a symlinked immediate skill
    // directory of that root.
    for lexical_root in agent_skills::global_skills_dirs() {
        let normalized_root = normalize_path(&lexical_root);
        let Some(canonical_root) = canonicalize_with_ancestors(&normalized_root, fs).await else {
            continue;
        };
