...
use agent_skills::{
    ...
    global_skills_dirs, load_skills_from_directory, parse_skill_frontmatter,
...
async fn expand_project_skills_directories(
    ...
    // Expand both project skill roots (`.agents/skills` and `.claude/skills`)
    // so their child SKILL.md files are scanned. Each root's parent (e.g.
    // `.agents`) is expanded first so the skills directory itself is visible.
    for skills_prefix in SKILLS_PREFIXES.iter() {
        if let Some(parent) = skills_prefix.parent() {
            expand_worktree_directory(worktree, parent, cx).await?;
        }
