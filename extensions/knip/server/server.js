...
var knipDiagnostics = /* @__PURE__ */ new Map();
var debounceTimer;
...
function makeRange(item) {
  const line = item.line ? item.line - 1 : 0;
  const col = item.col ? item.col - 1 : 0;
  return {
    start: import_node.Position.create(line, col),
    end: import_node.Position.create(line, col + item.name.length)
  };
}
function runKnip() {
  if (!workspaceRoot) {
...
function publishDiagnostics(report) {
  const newFiles = /* @__PURE__ */ new Set();
  const diagnosticsByFile = /* @__PURE__ */ new Map();
  for (const fileIssues of report.issues ?? []) {
    const filePath = path.isAbsolute(fileIssues.file) ? fileIssues.file : path.join(workspaceRoot, fileIssues.file);
    const fileUri = URI.file(filePath).toString();
    newFiles.add(fileUri);
    const diagnostics = diagnosticsByFile.get(fileUri) ?? [];
    for (const category of ISSUE_CATEGORIES) {
      const items = fileIssues[category.key];
      if (!items) continue;
...
connection.onCodeAction((params) => {
  ...
  for (const diagnostic of params.context.diagnostics) {
    ...
    const matched = fileDiagnostics.find(
      (d) => d.range.start.line === diagnostic.range.start.line && d.range.start.character === diagnostic.range.start.character && d.code === diagnostic.code
    );
    if (!matched) continue;
    ...
    if (code === "exports" || code === "types") {
      ...
      if (document) {
        const line = document.getText({
          start: import_node.Position.create(matched.range.start.line, 0),
          end: import_node.Position.create(matched.range.start.line + 1, 0)
        });
                ...
                [uri]: [
                  import_node.TextEdit.replace(
                    import_node.Range.create(
                      import_node.Position.create(matched.range.start.line, exportStart),
                      import_node.Position.create(matched.range.start.line, exportStart + 7)
                      // "export " = 7 chars
                    ),
                    ""
                  )
